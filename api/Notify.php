...
//print_r($purchases);
        $token = "575362163:AAGw__acYpwtgg_w4eyxFme0aO1gLcIPKok";
        
        $mess =  'Заказ: ' . $order->id . ', дата: ' . $order->date . ', имя: ' . $order->name . ', ' . ', телефон: ' . $order->phone . ', email:' . $order->email . ', цена:' . $order->total_price . ', доставка: ' . $delivery->name . ', ';
...
$tbot = file_get_contents("https://api.telegram.org/bot".$token."/sendMessage?chat_id=".$chatid."&text=".urlencode($mess));
        ...
        $this->email('zakaz@sportdream.com.ua', $subject, $email_template, $this->settings->notify_from_email);
