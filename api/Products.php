...
require_once(dirname(dirname(__FILE__)).'/lib/Slugify.class.php');
...
class Products extends Okay {
    ...
    public function get_products($filter = array()) {
        ...
        //'where p.id > (SELECT p.* product pp ORDER created DESC LIMIT 20)';
...
//echo $filter['sort'];
            switch ($filter['sort']) {
                ...
                case 'date_add':
                    $order = 'RAND()';
                    $new_filter = $this->db->placehold('AND p.created >= DATE_ADD(CURDATE(), INTERVAL -30 DAY)');
                    break;                    
                case 'price':
...
//Savchuk
            $order_stock = " (SELECT (IF(SUM(pv1.stock) = 0,1,0))
                    FROM __variants pv1
                    WHERE p.id = pv1.product_id
                )   
                ASC, ";

            $order = $order_stock . $order;
...
//Savchuk
        $price_filter_variant = "";
        if($filter['get_price']){
            $price_filter_variant = " AND pv.price > 0 ";
        }
...
//Savchuk
        if(empty($product['url'])) {
            $product['url'] = SlugifyClass::Slugify($product['name']);
        }
/*        
        if(empty($product['url'])) {
...
*/
    ...
    public function get_recommended_products($category_id) {
