...
//        if(empty($product) || (!$product->visible && empty($_SESSION['admin']))) {
        ...
        foreach( $this->products->get_recommended_products($category->id)  as $p) {
            $recommended_products[$p->id] = $p;
        }
            $related_products_images = $this->products->get_images(array('product_id'=>array_keys($recommended_products)));
            foreach($related_products_images as $related_product_image) {
                if(isset($recommended_products[$related_product_image->product_id])) {
                    $recommended_products[$related_product_image->product_id]->images[] = $related_product_image;
                }
            }
            $related_products_variants = $this->variants->get_variants(array('product_id'=>array_keys($recommended_products)));
            foreach($related_products_variants as $related_product_variant) {
                if(isset($recommended_products[$related_product_variant->product_id])) {
                    $recommended_products[$related_product_variant->product_id]->variants[] = $related_product_variant;
                }
            }
            foreach($recommended_products as $id=>$r) {
                if(is_object($r)) {
                    $r->image = $r->images[0];
                    $r->variant = $r->variants[0];
                } else {
                    unset($recommended_products[$id]);
                }
            }
        
        $this->design->assign('recommended_products', $recommended_products);
        ...
        $add_title = '≡ ' . $product->name . ' купить | Интернет магазин';
        $this->design->assign('meta_title', $add_title);
        
        $add_description = 'lll➤ ' . $product->name . ' купить в интернет магазине';
        $this->design->assign('meta_description', $add_description);
