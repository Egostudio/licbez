...
//Savchuk
        if (in_array('page-1',$this->uri_array)){
            $category_url = $this->request->get('category', 'string');
            $location = $this->config->root_url.'/'.$this->lang_link.'category/'.$category_url;
        ...
        foreach($this->uri_array as $k=>$v) {
            ...
            if(!$k && $brand=$this->brands->get_brand((string)$v)) {
                ...
                if (in_array($this->page->url, array('all-products','novye-tovary', 'discounted', 'bestsellers'))
    ...
    public function filter_chpu_url($params, &$smarty) {
        ...
        if(!empty($result_array['brand'])) {
            ...
            $this->brands_string_name = $this->filter_chpu_sort_brands_name($result_array['brand']); // - это с сортировкой по брендам
    ...
    private function filter_chpu_sort_brands_name($brands_urls = array()) {
        if(empty($brands_urls)) {
            return false;
        }
        $result = '';
    ...
    private function filter_chpu_parse_url() {
        ...
        preg_match("~^$this->subdir(/?$this->lang_label)?/?(category|all-products|novye-tovary|brands|discounted|bestsellers)/?~", $uri['path'], $this->catalog_type);
        $this->catalog_type = $this->catalog_type[2];
        //убираем модификатор каталога
        $uri = preg_replace("~^$this->subdir(/?$this->lang_label)?/?(category|all-products|novye-tovary|brands|discounted|bestsellers)/?~",'',$uri['path']);
        $this->uri_array = (empty($uri) ? array() : explode('/',$uri));
        if ($this->catalog_type == 'category' || $this->catalog_type == 'brands') {
...
//print_r($this->features_urls);
        ...
        if ($val = $this->request->get('b')) {
            ...
            $brand = $this->brands->get_brand((string)$brand_url,1);
            if (empty($brand)) {
                return false;
            }
...
//Savchuk
        $category1 = $this->request->get('category', 'string');
        if($category1 != $_SESSION['category1']){
            $filter['sort'] = 'position';
        }
        ...
        if(!empty($category)) {
            ...
            foreach($this->features->get_features(array('category_id'=>$category->id, 'in_filter'=>1)) as $feature) {
                ...
                if($val = $this->request->get($feature->id)) {
                    ...
                    $filter1[$feature->id]['name'] = $feature->name;  
                    $val2 = array();
                    foreach($val as $val1){
                        $val2[$val1]=$val1;
                    }
                    $filter1[$feature->id]['val'] = $val2;
                }
            }
            $attr_f = ($attr_f || count($filter['features'])) ? true : false;
           ...
           //print_r($options_filter['feature_id']); 
            ...
            foreach($options as $option) {
                if(isset($features[$option->feature_id])) {
                    ...
                    if(isset($filter1[$option->feature_id]['val'][$option->translit])){
                        $filter1[$option->feature_id]['val'][$option->translit] = $option->value;
                    }
            ...
            $this->design->assign('attr_f', $attr_f);
        ...
        $this->design->assign('page_all', $page_all);
        ...
        $price_min = $prices->range->min;        
        $price_max = $prices->range->max;
        ...
        $design = $this->filter_chpu_url(array('sort'=>null), $this->design);
       
        // Устанавливаем мета-теги в зависимости от запроса

        $tcp = $current_page > 1 ? 'Страница ' . $current_page .'. ' : '' ;
        
        if($this->page) {
            $this->design->assign('meta_title', $tcp . $this->page->meta_title);
//            $this->design->assign('meta_keywords', $this->page->meta_keywords);
            $this->design->assign('meta_description', $tcp . $this->page->meta_description);
        } elseif(isset($category)) {

//meta_title

            $features_str = '';
            foreach($filter1 as $filter2){
                $features_str .= $filter2['name'] . ' - ';
                foreach($filter2['val'] as $filter3){
                    $features_str .= $filter3 . ', ';
                }
            }
            $features_str = substr($features_str, 0, strlen($features_str)-2) . " " ;
            $meta_title = $tcp . '≡ '. $category->name . ' ' . $this->brands_string_name . " " . $features_str . "купить | Интернет магазин";
            $meta_description =  $tcp . 'lll➤ ' . $category->name . ' ' .$this->brands_string_name . ' ' . $features_str . 'купить недорого в интернет магазине  ⭐Большой выбор: ' . $products_count .' шт. в наличии ✅Доступная цена от ' . $price_min . ' грн до ' . $price_max . ' грн ⌚Быстрый заказ ✈Отправка в день заказа. ✍Настоящие отзывы на товар ✅Доставка в Киев | Днепр | Харьков | Одесса | Львов и другие города Украины.';

            $this->design->assign('meta_title', $meta_title);
            $this->design->assign('meta_description', $meta_description);
            
        } elseif(isset($brand)) {
            $brand_meta_title = $tcp . '≡ Производитель ' . $brand->name . " каталог товаров купить в интернет магазине";
            $brand_meta_description =  $tcp . 'lll➤ Купить недорого товары бренда ' . $brand->name . ' в интернет магазине ⭐Большой выбор: ' . $products_count .' шт. в наличии ✅Доступная цена от ' . $price_min . ' грн до ' . $price_max . ' грн ⌚Быстрый заказ ✈Отправка в день заказа. ✍Настоящие отзывы на товар ✅Доставка в Киев | Днепр | Харьков | Одесса | Львов и другие города Украины.';   

            $this->design->assign('meta_title', $brand_meta_title);
            $this->design->assign('meta_description', $brand_meta_description);

        } elseif(isset($keyword)) {
            $this->design->assign('meta_title', $tcp . $keyword);
        }

        $rel_prev_next = $this->design->fetch('products_rel_prev_next.tpl');
        $this->design->assign('rel_prev_next', $rel_prev_next);
        $this->design->assign('sort_canonical', $design);
