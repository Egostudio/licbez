...
$(function () {
    $('.filters.tablet-hidden').find('.filter_group').each(function(){
        var filter = $(this);

        var item = filter.find('.filter_item');
        var item_l = item.length;

        if ( item_l > 7 ) {
            var tmpl = '<div class="hidden-filter-wrap">\
                            <div class="hidden-item"></div>\
                            <span class="show-more">Показать еще</span>\
                            <span class="hide-cat"style="display: none;">Скрыть</span>\
                        </div>';
            filter.append(tmpl);

            var hidden = filter.find('.hidden-item');
            var show = filter.find('.show-more');
            var hide = filter.find('.hide-cat');
    ...
    var description_text_wrap = $('#description'),
        description_text = description_text_wrap.find('.text'),
        description_text_h = description_text.height(),
        show_more_d = description_text_wrap.find('.show-more');

    if ( description_text_h > 250 ) {
        show_more_d.show();
        description_text.height(255);

        show_more_d.click(function() {
            if ( $(this).hasClass('open') ) {
                description_text.height(255);
            } else {
                description_text.height(description_text_h);
            }
