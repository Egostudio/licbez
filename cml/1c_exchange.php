<?php
//mail('iv.savchuk@gmail.com', 'sub', 'mess');
    ...
    if($filename === 'import.xml') {
        ...
        while($z->name === 'Товар') {
            if($current_product_num >= $last_product_num) {
                ...
                // Товары
                import_product($xml,$current_product_num);
...
//Savchuk
                $url = SlugifyClass::Slugify($xml_group->Наименование);
                //'url'=>$okay->translit($xml_group->Наименование)   

                $category_id = $okay->categories->add_category(array('parent_id'=>$parent_id, 'external_id'=>$xml_group->Ид, 'url'=>$url, 'name'=>$xml_group->Наименование, 'meta_title'=>$xml_group->Наименование, 'meta_keywords'=>$xml_group->Наименование, 'meta_description'=>$xml_group->Наименование ));
            }
            else{
//Savchuk            
//Постоянное обновление категорий, проверка на предмет наименования родителськой категории            
                $okay->categories->update_category($category_id, array('parent_id'=>$parent_id,'name'=>$xml_group->Наименование));
            }
...
function import_product($xml_product,$current_product_num) {
    global $okay;
    global $dir;
    global $brand_option_name;
	global $full_update;
...
//      'url'=>$okay->translit($xml_product->Наименование),
        $url = SlugifyClass::Slugify($xml_product->Наименование);
        ...
        if(isset($xml_product->Картинка)) {
            ...
            foreach($xml_product->Картинка as $img) {
                ...
                if(!empty($image) && is_file($dir.$image) && is_writable($okay->config->original_images_dir)) {
                    ...
                    $without_extension  = substr($image, 0, strrpos($image, '.'));
                    $without_extension  = SlugifyClass::Slugify($without_extension);
                    $image2 = $without_extension . $extension;

                    rename($dir.$image, $okay->config->original_images_dir.$image2);
                    $okay->products->add_image($product_id, $image2);
                }
            }
        }
    }
...
//        if($current_product_num==0){
            ...
            // Удаляем изображения
            $images = $okay->products->get_images(array('product_id'=>$product_id));
            foreach($images as $i) {
                $okay->products->delete_image($i->id);
            }
            ...
            foreach($xml_product->Картинка as $img) {
                ...
                //mail('iv.savchuk@gmail.com', 'sub', $without_extension . ' -- '. $extension); exit;
...
//        $variant->stock = 100;
    ...
    if($xml_product->Статус == 'Удален') {
        ...
        $okay->products->update_product($product_id, array('visible'=>0));
...
//Savchuk
//Скидка
    foreach($xml_variant->Скидки->Скидка as $xml_discount) {
    	$variant->discount = $xml_discount->Процент;
    	$variant->compare_price = $variant->price;
    	$variant->price = floor($variant->price*(1-$variant->discount/100));
    }
    ...
    if ($stock_from_1c) {
        $stock = $xml_variant->Количество;
		$variant->stock = $stock;
        //$variant->stock = $stock > 0 ? 100 : 0 ;
        //$variant->stock = 100;
    } else {
        $variant->stock = NULL;
//	$variant->stock = 100;
    ...
    if(empty($variant_id)) {
        ...
        //error_log("<BR>====== ".date("d.m.Y H:i:s")." ====== update $variant->stock<BR> $stock <BR>", 3, "cml/temp/1c_exchange.log.htm");
        $okay->variants->update_variant($variant_id, $variant);
    }
}
