            ...
            <div class="boxed fn_toggle_wrap min_height_230px">
                ...
                <div class="toggle_body_wrap on fn_card">
                    <div class="box_border_buyer">
                        ...
                        {*novaposhta_ttn*}
                        <div class="mb-1">
                            <div class="heading_label">{$btr->general_lastname|escape}</div>
                            <input name="lastname" class="form-control" type="text" value="{$order->lastname|escape}" />
                        </div>
                        {*/novaposhta_ttn*}
                        <div class="mb-1">
                        ...
                        <div class="row">
                            <div class="col-lg-6 col-md-6">
                                <div class="heading_label">{$btr->general_city|escape}</div>
                                <div class="edit_order_detail" style='display: none;'>
                                    <select name="city" data-placeholder="" style="width:350px;" tabindex="1" class="npcity"></select>
                                </div>
                                <div class="view_order_detail city">
                                    {$order->city|escape}
                                </div>
                            </div>
                            <div class="col-lg-6 col-md-6">
                                <div class="heading_label">{$btr->general_warehouse|escape}</div>
                                <div class="edit_order_detail" style='display: none;'>
                                    <select name="np_warehouse" data-placeholder="" class="npware"></select>
                                </div>
                                <div class="view_order_detail warehouse" >
                                    {$order->np_warehouse|escape}
                                </div>
                            </div>
                        </div>
                        <button type="button" class="btn btn_city btn_mini btn-info fn_np_change_address">
                            {include file='svg_icon.tpl' svgId='plus'}
                            <span>{$btr->general_change_info|escape}</span>
                        </button>
                        {*/novaposhta_ttn*}
                        <div class="mb-1">
                    ...
                    <div class="box_border_buyer">
                        {if $order->ttn_ref}
                            <a href="https://my.novaposhta.ua/newOrder/index/Ref/{$order->ttn_ref}" target="_blank">ТТН №{$order->ttn_num}</a>
                        {else}
                            <div class="mb-1">
                                <div class="checkbox_email text_dark mr-1">
                                    <input id="is_back_delivery" name="is_back_delivery" type="checkbox" class="hidden_check_1"  value="1" />
                                    <label for="is_back_delivery" class="checkbox_label mr-h"></label>
                                    <span>Указать обратную доставку</span>
                                </div>
                            </div>
                            ...
                            <a id="ttn_link" href="{url ttn=1 back_delivery=1 payer_type=$settings->np_payer_type id=$order->id}">Сгенерировать ТТН</a>
                        {/if}
                    </div>
                    {*/novaposhta_ttn*}
                </div>
...
<script src ="design/js/maskeditinput/jquery.maskedinput.js" type="text/javascript"></script>
<!-- /novaposhta_ttn -->
...
$(document).ready(function($){
    $('.to_generate_url').change(function() {
        /*parser*/
        a_ttn = $('#ttn_link');
        parser = document.createElement('a');
        parser.href = a_ttn.attr('href');
        tm = parser.search.substring(1).split('&');
        new_href = '?';
        if (tm.length > 0) {
            for (var i in tm) {
                if (tm[i].indexOf('back_delivery') == -1 && tm[i].indexOf('payer_type') == -1) {
                    new_href += tm[i] + '&';
                }
            }
            new_href = new_href.substring(0, new_href.length-1);
            is_checkbox = $(this).is('[type="checkbox"]');
            if ($('#is_back_delivery').is(':checked')) {
                new_href += '&back_delivery=1';
            }
            new_href += '&payer_type=' + $('input[name="np_payer_type"]:checked').val();
            a_ttn.attr('href', new_href);
        }
        /*/parser*/
    });
...
/*
    ...
    $('input[name="phone"]').val({/literal}'{$order->phone}'{literal});
    $.ajax({
        url: "ajax/np.php",
        dataType: 'json',
        success: function(data) {
            ...
            $(".npcity").show();
            $('select.npcity option[value="' + {/literal}'{$order->city|escape}'{literal} + '"]').attr('selected', true).trigger('change').val();
            $('.view_order_detail.city').html($('select.npcity option[value="' + {/literal}'{$order->city|escape}'{literal} + '"]').html());
        }
    });
*/
    $("select.npcity").change(function() {
        ...
        if(city_ref != '' && city_ref) {
            ...
            $('select.npware option[value="' + {/literal}'{$order->np_warehouse|escape}'{literal} + '"]').attr('selected', true).trigger('change');
            $('.view_order_detail.warehouse').html($('select.npware option[value="' + {/literal}'{$order->np_warehouse|escape}'{literal} + '"]').html());
        }
    });

    setInterval(function() {
        if ($('.npcity').children('option').length>1) {
            $('.npcity').chosen({width: "100%",no_results_text: "Вашего города нет в списке"});
        };
    }, 100);

    $(document).on("click",".fn_np_change_address",function () {
        $('.edit_order_detail').show();
    });
    /*/novaposhta_ttn*/
